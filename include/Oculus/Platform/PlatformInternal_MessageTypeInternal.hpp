// Autogenerated from CppHeaderCreator
// Created by Sc2ad
// =========================================================================
#pragma once
// Begin includes
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "beatsaber-hook/shared/utils/byref.hpp"
// Including type: Oculus.Platform.PlatformInternal
#include "Oculus/Platform/PlatformInternal.hpp"
// Including type: System.Enum
#include "System/Enum.hpp"
// Completed includes
#include "beatsaber-hook/shared/utils/il2cpp-type-check.hpp"
DEFINE_IL2CPP_ARG_TYPE(::Oculus::Platform::PlatformInternal::MessageTypeInternal, "Oculus.Platform", "PlatformInternal/MessageTypeInternal");
// Type namespace: Oculus.Platform
namespace Oculus::Platform {
  // Size: 0x14
  #pragma pack(push, 1)
  // Autogenerated type: Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal
  // [TokenAttribute] Offset: FFFFFFFF
  struct PlatformInternal::MessageTypeInternal/*, public ::System::Enum*/ {
    public:
    // Writing base type padding for base size: 0x0 to desired offset: 0x10
    char ___base_padding[0x10] = {};
    public:
    // public System.UInt32 value__
    // Size: 0x4
    // Offset: 0x10
    uint value;
    // Field size check
    static_assert(sizeof(uint) == 0x4);
    public:
    // Creating value type constructor for type: MessageTypeInternal
    constexpr MessageTypeInternal(uint value_ = {}) noexcept : value{value_} {}
    // Creating interface conversion operator: operator ::System::Enum
    operator ::System::Enum() noexcept {
      return *reinterpret_cast<::System::Enum*>(this);
    }
    // Creating conversion operator: operator uint
    constexpr operator uint() const noexcept {
      return value;
    }
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal AbuseReport_LaunchAdvancedReportFlow
    static constexpr const uint AbuseReport_LaunchAdvancedReportFlow = 1286683246u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal AbuseReport_LaunchAdvancedReportFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_AbuseReport_LaunchAdvancedReportFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal AbuseReport_LaunchAdvancedReportFlow
    static void _set_AbuseReport_LaunchAdvancedReportFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_ExecuteCoordinatedLaunch
    static constexpr const uint Application_ExecuteCoordinatedLaunch = 645772532u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_ExecuteCoordinatedLaunch
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Application_ExecuteCoordinatedLaunch();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_ExecuteCoordinatedLaunch
    static void _set_Application_ExecuteCoordinatedLaunch(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_GetInstalledApplications
    static constexpr const uint Application_GetInstalledApplications = 1376744524u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_GetInstalledApplications
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Application_GetInstalledApplications();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Application_GetInstalledApplications
    static void _set_Application_GetInstalledApplications(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Avatar_UpdateMetaData
    static constexpr const uint Avatar_UpdateMetaData = 2077219214u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Avatar_UpdateMetaData
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Avatar_UpdateMetaData();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Avatar_UpdateMetaData
    static void _set_Avatar_UpdateMetaData(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_FinalizeApplication
    static constexpr const uint Cal_FinalizeApplication = 497667029u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_FinalizeApplication
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Cal_FinalizeApplication();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_FinalizeApplication
    static void _set_Cal_FinalizeApplication(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_GetSuggestedApplications
    static constexpr const uint Cal_GetSuggestedApplications = 1450209301u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_GetSuggestedApplications
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Cal_GetSuggestedApplications();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_GetSuggestedApplications
    static void _set_Cal_GetSuggestedApplications(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_ProposeApplication
    static constexpr const uint Cal_ProposeApplication = 1317270237u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_ProposeApplication
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Cal_ProposeApplication();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Cal_ProposeApplication
    static void _set_Cal_ProposeApplication(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_GetCurrentMapUuid
    static constexpr const uint Colocation_GetCurrentMapUuid = 878018226u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_GetCurrentMapUuid
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Colocation_GetCurrentMapUuid();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_GetCurrentMapUuid
    static void _set_Colocation_GetCurrentMapUuid(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_RequestMap
    static constexpr const uint Colocation_RequestMap = 840263277u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_RequestMap
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Colocation_RequestMap();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_RequestMap
    static void _set_Colocation_RequestMap(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_ShareMap
    static constexpr const uint Colocation_ShareMap = 409847005u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_ShareMap
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Colocation_ShareMap();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Colocation_ShareMap
    static void _set_Colocation_ShareMap(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Get
    static constexpr const uint GraphAPI_Get = 822018158u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Get
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_GraphAPI_Get();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Get
    static void _set_GraphAPI_Get(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Post
    static constexpr const uint GraphAPI_Post = 1990567876u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Post
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_GraphAPI_Post();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal GraphAPI_Post
    static void _set_GraphAPI_Post(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Get
    static constexpr const uint HTTP_Get = 1874211363u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Get
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_HTTP_Get();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Get
    static void _set_HTTP_Get(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_GetToFile
    static constexpr const uint HTTP_GetToFile = 1317133401u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_GetToFile
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_HTTP_GetToFile();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_GetToFile
    static void _set_HTTP_GetToFile(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_MultiPartPost
    static constexpr const uint HTTP_MultiPartPost = 1480774160u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_MultiPartPost
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_HTTP_MultiPartPost();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_MultiPartPost
    static void _set_HTTP_MultiPartPost(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Post
    static constexpr const uint HTTP_Post = 1798743375u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Post
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_HTTP_Post();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal HTTP_Post
    static void _set_HTTP_Post(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_IsAllowedForApplication
    static constexpr const uint Livestreaming_IsAllowedForApplication = 191729014u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_IsAllowedForApplication
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_IsAllowedForApplication();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_IsAllowedForApplication
    static void _set_Livestreaming_IsAllowedForApplication(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartPartyStream
    static constexpr const uint Livestreaming_StartPartyStream = 2066701532u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartPartyStream
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_StartPartyStream();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartPartyStream
    static void _set_Livestreaming_StartPartyStream(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartStream
    static constexpr const uint Livestreaming_StartStream = 1343932350u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartStream
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_StartStream();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StartStream
    static void _set_Livestreaming_StartStream(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopPartyStream
    static constexpr const uint Livestreaming_StopPartyStream = 661065560u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopPartyStream
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_StopPartyStream();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopPartyStream
    static void _set_Livestreaming_StopPartyStream(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopStream
    static constexpr const uint Livestreaming_StopStream = 1155796426u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopStream
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_StopStream();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_StopStream
    static void _set_Livestreaming_StopStream(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_UpdateMicStatus
    static constexpr const uint Livestreaming_UpdateMicStatus = 475495815u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_UpdateMicStatus
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Livestreaming_UpdateMicStatus();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Livestreaming_UpdateMicStatus
    static void _set_Livestreaming_UpdateMicStatus(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Connect
    static constexpr const uint NetSync_Connect = 1684899167u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Connect
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_Connect();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Connect
    static void _set_NetSync_Connect(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Disconnect
    static constexpr const uint NetSync_Disconnect = 359268021u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Disconnect
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_Disconnect();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_Disconnect
    static void _set_NetSync_Disconnect(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetSessions
    static constexpr const uint NetSync_GetSessions = 1859521077u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetSessions
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_GetSessions();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetSessions
    static void _set_NetSync_GetSessions(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuation
    static constexpr const uint NetSync_GetVoipAttenuation = 288016919u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuation
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_GetVoipAttenuation();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuation
    static void _set_NetSync_GetVoipAttenuation(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuationDefault
    static constexpr const uint NetSync_GetVoipAttenuationDefault = 1467721888u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuationDefault
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_GetVoipAttenuationDefault();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_GetVoipAttenuationDefault
    static void _set_NetSync_GetVoipAttenuationDefault(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuation
    static constexpr const uint NetSync_SetVoipAttenuation = 882366454u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuation
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipAttenuation();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuation
    static void _set_NetSync_SetVoipAttenuation(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuationModel
    static constexpr const uint NetSync_SetVoipAttenuationModel = 1788128654u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuationModel
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipAttenuationModel();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipAttenuationModel
    static void _set_NetSync_SetVoipAttenuationModel(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipChannelCfg
    static constexpr const uint NetSync_SetVoipChannelCfg = 1553310963u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipChannelCfg
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipChannelCfg();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipChannelCfg
    static void _set_NetSync_SetVoipChannelCfg(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipGroup
    static constexpr const uint NetSync_SetVoipGroup = 1477614734u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipGroup
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipGroup();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipGroup
    static void _set_NetSync_SetVoipGroup(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipListentoChannels
    static constexpr const uint NetSync_SetVoipListentoChannels = 1590749746u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipListentoChannels
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipListentoChannels();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipListentoChannels
    static void _set_NetSync_SetVoipListentoChannels(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipMicSource
    static constexpr const uint NetSync_SetVoipMicSource = 855832432u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipMicSource
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipMicSource();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipMicSource
    static void _set_NetSync_SetVoipMicSource(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSessionMuted
    static constexpr const uint NetSync_SetVoipSessionMuted = 1434844938u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSessionMuted
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipSessionMuted();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSessionMuted
    static void _set_NetSync_SetVoipSessionMuted(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSpeaktoChannels
    static constexpr const uint NetSync_SetVoipSpeaktoChannels = 766496213u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSpeaktoChannels
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipSpeaktoChannels();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipSpeaktoChannels
    static void _set_NetSync_SetVoipSpeaktoChannels(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipStreamMode
    static constexpr const uint NetSync_SetVoipStreamMode = 1742839095u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipStreamMode
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_NetSync_SetVoipStreamMode();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal NetSync_SetVoipStreamMode
    static void _set_NetSync_SetVoipStreamMode(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Create
    static constexpr const uint Party_Create = 450042703u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Create
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_Create();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Create
    static void _set_Party_Create(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GatherInApplication
    static constexpr const uint Party_GatherInApplication = 1921499523u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GatherInApplication
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_GatherInApplication();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GatherInApplication
    static void _set_Party_GatherInApplication(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Get
    static constexpr const uint Party_Get = 1586058173u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Get
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_Get();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Get
    static void _set_Party_Get(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GetCurrentForUser
    static constexpr const uint Party_GetCurrentForUser = 1489764138u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GetCurrentForUser
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_GetCurrentForUser();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_GetCurrentForUser
    static void _set_Party_GetCurrentForUser(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Invite
    static constexpr const uint Party_Invite = 901104867u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Invite
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_Invite();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Invite
    static void _set_Party_Invite(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Join
    static constexpr const uint Party_Join = 1744993395u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Join
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_Join();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Join
    static void _set_Party_Join(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Leave
    static constexpr const uint Party_Leave = 848430801u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Leave
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Party_Leave();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Party_Leave
    static void _set_Party_Leave(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetDestination
    static constexpr const uint RichPresence_SetDestination = 1328734477u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetDestination
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_RichPresence_SetDestination();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetDestination
    static void _set_RichPresence_SetDestination(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetIsJoinable
    static constexpr const uint RichPresence_SetIsJoinable = 1050353505u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetIsJoinable
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_RichPresence_SetIsJoinable();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetIsJoinable
    static void _set_RichPresence_SetIsJoinable(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetLobbySession
    static constexpr const uint RichPresence_SetLobbySession = 1895893271u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetLobbySession
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_RichPresence_SetLobbySession();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetLobbySession
    static void _set_RichPresence_SetLobbySession(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetMatchSession
    static constexpr const uint RichPresence_SetMatchSession = 1675623566u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetMatchSession
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_RichPresence_SetMatchSession();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal RichPresence_SetMatchSession
    static void _set_RichPresence_SetMatchSession(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_CreateOrUpdateAndJoinNamed
    static constexpr const uint Room_CreateOrUpdateAndJoinNamed = 2089683601u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_CreateOrUpdateAndJoinNamed
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Room_CreateOrUpdateAndJoinNamed();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_CreateOrUpdateAndJoinNamed
    static void _set_Room_CreateOrUpdateAndJoinNamed(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetNamedRooms
    static constexpr const uint Room_GetNamedRooms = 125660812u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetNamedRooms
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Room_GetNamedRooms();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetNamedRooms
    static void _set_Room_GetNamedRooms(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetSocialRooms
    static constexpr const uint Room_GetSocialRooms = 1636310390u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetSocialRooms
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_Room_GetSocialRooms();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal Room_GetSocialRooms
    static void _set_Room_GetSocialRooms(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_CancelRecordingForReportFlow
    static constexpr const uint User_CancelRecordingForReportFlow = 65065289u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_CancelRecordingForReportFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_CancelRecordingForReportFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_CancelRecordingForReportFlow
    static void _set_User_CancelRecordingForReportFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_GetLinkedAccounts
    static constexpr const uint User_GetLinkedAccounts = 1469314134u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_GetLinkedAccounts
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_GetLinkedAccounts();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_GetLinkedAccounts
    static void _set_User_GetLinkedAccounts(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchBlockFlow
    static constexpr const uint User_LaunchBlockFlow = 1876305192u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchBlockFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_LaunchBlockFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchBlockFlow
    static void _set_User_LaunchBlockFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow
    static constexpr const uint User_LaunchReportFlow = 1449304081u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_LaunchReportFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow
    static void _set_User_LaunchReportFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow2
    static constexpr const uint User_LaunchReportFlow2 = 2139314275u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow2
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_LaunchReportFlow2();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchReportFlow2
    static void _set_User_LaunchReportFlow2(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchUnblockFlow
    static constexpr const uint User_LaunchUnblockFlow = 346172055u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchUnblockFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_LaunchUnblockFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_LaunchUnblockFlow
    static void _set_User_LaunchUnblockFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewEntitledTestUser
    static constexpr const uint User_NewEntitledTestUser = 292822787u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewEntitledTestUser
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_NewEntitledTestUser();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewEntitledTestUser
    static void _set_User_NewEntitledTestUser(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUser
    static constexpr const uint User_NewTestUser = 921194380u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUser
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_NewTestUser();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUser
    static void _set_User_NewTestUser(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUserFriends
    static constexpr const uint User_NewTestUserFriends = 517416647u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUserFriends
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_NewTestUserFriends();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_NewTestUserFriends
    static void _set_User_NewTestUserFriends(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StartRecordingForReportFlow
    static constexpr const uint User_StartRecordingForReportFlow = 1819161571u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StartRecordingForReportFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_StartRecordingForReportFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StartRecordingForReportFlow
    static void _set_User_StartRecordingForReportFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow
    static constexpr const uint User_StopRecordingAndLaunchReportFlow = 1618513035u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_StopRecordingAndLaunchReportFlow();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow
    static void _set_User_StopRecordingAndLaunchReportFlow(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow2
    static constexpr const uint User_StopRecordingAndLaunchReportFlow2 = 432190251u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow2
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_StopRecordingAndLaunchReportFlow2();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_StopRecordingAndLaunchReportFlow2
    static void _set_User_StopRecordingAndLaunchReportFlow2(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // static field const value: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_TestUserCreateDeviceManifest
    static constexpr const uint User_TestUserCreateDeviceManifest = 1701884605u;
    // Get static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_TestUserCreateDeviceManifest
    static ::Oculus::Platform::PlatformInternal::MessageTypeInternal _get_User_TestUserCreateDeviceManifest();
    // Set static field: static public Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal User_TestUserCreateDeviceManifest
    static void _set_User_TestUserCreateDeviceManifest(::Oculus::Platform::PlatformInternal::MessageTypeInternal value);
    // Get instance field reference: public System.UInt32 value__
    uint& dyn_value__();
  }; // Oculus.Platform.PlatformInternal/Oculus.Platform.MessageTypeInternal
  #pragma pack(pop)
  static check_size<sizeof(PlatformInternal::MessageTypeInternal), 16 + sizeof(uint)> __Oculus_Platform_PlatformInternal_MessageTypeInternalSizeCheck;
  static_assert(sizeof(PlatformInternal::MessageTypeInternal) == 0x14);
}
#include "beatsaber-hook/shared/utils/il2cpp-utils-methods.hpp"
